...
- (void) addMessageToDisplay:(NSData *) message fromUser:(MVChatUser *) user withAttributes:(NSDictionary *) msgAttributes withIdentifier:(NSString *) identifier andType:(JVChatMessageType) type {
	...
	if( ! [[NSUserDefaults standardUserDefaults] boolForKey:@"MVChatDisableLinkHighlighting"] ) {
		hasLinks = [messageString makeLinkAttributesAutomatically];
...
//	[_sqlTestTranscript appendMessage:cmessage];
	...
	if( [cmessage ignoreStatus] == JVNotIgnored ) {
		...
		else text = [NSString stringWithFormat:@"%@ says %@", [[cmessage sender] displayName], text];
		
		if( [voiceIdentifier length] ) {
			[[JVSpeechController sharedSpeechController] startSpeakingString:text usingVoice:voiceIdentifier];
		} else {
			// Don't read messages with links, it sounds like chaos
			if( [[NSUserDefaults standardUserDefaults] boolForKey:@"allTextToSpeech"] && ! hasLinks) {
				BOOL read = false;
				if ([[[NSUserDefaults standardUserDefaults] stringForKey:@"speechOnlyInRoom"] length]) {
					if ([[[NSUserDefaults standardUserDefaults] stringForKey:@"speechOnlyInRoom"] isEqualToString:[[self target] name]]) {
						read = true;
					}
				} else {
					read = true;
				...
				if (read)
					[[JVSpeechController sharedSpeechController] startSpeakingString:text forUser:[[cmessage sender] displayName]];
			}
		}
	}
